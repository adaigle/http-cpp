...
std::string http_constants::reason_phrase(uint16_t code)
{
    ...
    default:
        throw std::invalid_argument("The HTTP status code provided is invalid.");
    }
...
http_request::parsing_status http_request::parse(std::string& frame)
{
    ...
    const auto find_iter = std::find(std::cbegin(http_constants::METHODS), std::cend(http_constants::METHODS), raw_method);
    ...
    for (std::string line; std::getline(input, line) && !line.empty();) {
        ...
        const auto general_iter = std::find(std::cbegin(http_constants::GENERAL_HEADER), std::cend(http_constants::GENERAL_HEADER), header);
        const auto request_iter = std::find(std::cbegin(http_constants::REQUEST_HEADER), std::cend(http_constants::REQUEST_HEADER), header);
        const auto entity_iter = std::find(std::cbegin(http_constants::ENTITY_HEADER), std::cend(http_constants::ENTITY_HEADER), header);
